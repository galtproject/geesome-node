...
- Keep default test/start scripts quiet. Use `DEBUG=geesome*` only when module traces are useful, `GEESOME_TEST_LOGS=1` only when test stdout diagnostics are needed, `GEESOME_ACCESS_LOGS=1` only when HTTP access logs are needed, and `GEESOME_LOG_SQL=1 DEBUG=geesome:database:sql` only when SQL trace output is needed.
