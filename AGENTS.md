...
- For frontend publisher/cache changes, run `npm run test:frontend-dist-publish` and `npm run test:frontend-cache:docker`; the latter verifies immutable image output reuse across fresh containers with a persistent publication volume.
