...
- Integrate ActivityPub/Fediverse so GeeSome groups and posts can federate with compatible servers; the current MVP has public actor/outbox/post/follower/following/follow routes, signed inbound follow acceptance/cancellation, outbound follow-accept delivery, local post ActivityPub object records, publish-time `Create(Note)` delivery queues, shared-inbox remote object records for replies to known local objects, and shared-inbox `Delete`/`Undo(Create)` tombstones for cached remote objects, with outbound follows and broader remote post creation/update/delete handling still to finish.
