...
- [Agent docs map](./docs/agent-map.md)
...
# Debugging and logs
Logs are quiet by default. See [DEBUG.md](DEBUG.md) for debug namespaces, log flags, and memory profiling (including how to dump per-process memory to a file for sizing hardware requirements).
...
- Integrate ActivityPub/Fediverse so GeeSome groups and posts can federate with compatible servers; the current MVP has public WebFinger/NodeInfo discovery, actor/outbox/post/follower/following/follow routes, signed inbound follow acceptance/cancellation/block handling, signed inbound flag-report storage plus admin listing/state/target context, outbound follow request/follow-accept delivery queues, signed remote accept/reject response handling for outbound follows, local post ActivityPub object records, publish-time `Create(Note)` delivery queues, shared-inbox remote object records for replies to known local objects and mentions of known local group actors, review-only shared-inbox `Create`/`Update` caching for broader first-class ActivityStreams object types, admin remote-object review with sanitized preview fields, bounded remote attachment/media metadata, per-attachment embed policy and backup eligibility, and an explicit remote attachment import policy, accepted-review-gated post-draft projection and manual native-post creation from accepted public Notes with local reply threading plus opt-in HTTP(S)/IPFS/IPNS media/document attachment backup and an async retry queue for missing supported backups, persistent review-state decisions, review-state filtering, shared-inbox `Update`, `Delete`, and `Undo(Create)` handling for cached remote objects, linked imported-post update on signed remote Note changes, linked imported-post soft-delete on signed remote tombstones, a geesome-ui admin review screen for escaped previews/review-state changes/manual post creation, a local deterministic interop smoke check for discovery/actor/outbox/object payloads, a generic live remote-server smoke for configurable Fediverse actor/public Note review-import behavior, a live operator-run Bluesky-through-Bridgy-Fed smoke for public bridge discovery plus local shared-inbox storage/review/post-create behavior, an ActivityPub source subscription/feed UI with e2e coverage, native Bluesky public source subscriptions/feed reads plus bounded AT-record sync for changed/deleted imported posts, user-scoped native Bluesky account login/verification, backend text/rich-text plus supported-image Bluesky cross-posting with idempotent URI/CID storage, explicit Bluesky cross-post and import relation/media policy controls with stored source-subscription defaults and source-subscription/cross-post/social-migration UI exposure plus social-migration reconciliation status/actions, stronger actor-signed ActivityPub ownership challenges for claimed visible social migration, and a canonical rich-text format so raw HTML stays adapter output rather than native post source. Remaining finish work is signed ownership challenge operability hardening, deeper conformance/credentialed live smoke, and native rich-text storage/editor integration.
...
See [docs/todo.md](./docs/todo.md) for the issue-backed backlog snapshot and fast delivery plan.
...
- API [documentation](https://geesome-node.galtproject.io:7722/ipns/QmQdr2fJ1qsYHnGN2FC1G9kitypiUQ271GhLa8qEtJnSN7/) and [endpoints code](./app/modules/api/index.ts)
