...
- Integrate ActivityPub/Fediverse so GeeSome groups and posts can federate with compatible servers; the current MVP has public actor/outbox/post/follow routes, signed inbound follow acceptance, outbound follow-accept delivery, local post ActivityPub object records, publish-time `Create(Note)` delivery queues, and shared-inbox remote object records for replies to known local objects, with broader remote post creation/update/delete handling still to finish.
