...
- Add a global storage space analyzer UI and restored-database reports for file-catalog and group usage, with drilldowns into largest files, file types, duplicate/shared storage IDs, safe cleanup blockers, queued physical storage cleanup, and a separate IPFS popularity/availability tab for peer/provider and retrieval signals.
