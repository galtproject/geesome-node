...
- Integrate ActivityPub/Fediverse so GeeSome groups and posts can federate with compatible servers; the current MVP has public actor/outbox/post/follower/following/follow routes, signed inbound follow acceptance/cancellation/block handling, signed inbound flag-report storage plus admin listing/state updates, outbound follow request and follow-accept delivery queues, local post ActivityPub object records, publish-time `Create(Note)` delivery queues, shared-inbox remote object records for replies to known local objects, and shared-inbox `Update(Note)`, `Delete`, and `Undo(Create)` handling for cached remote objects, with outbound follow accept/reject handling and broader visible remote post creation/update/delete handling still to finish.
