...
- Integrate ActivityPub/Fediverse so GeeSome groups and posts can federate with compatible servers; the current MVP has public actor/outbox/post/follow routes, signed inbound follow acceptance, outbound follow-accept delivery, local post ActivityPub object records, and publish-time `Create(Note)` delivery queues, with shared-inbox object routing still to finish.
