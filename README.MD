...
```
This runs `bash/docker-rebuild-and-upgrade.sh`: it pulls the latest source, selects a verified published image for that Git commit (or builds locally when unavailable), and restarts the `geesome-docker` systemd service.
...
- Concurrent video transcoding or large imports need extra headroom. You can profile your own load with the memory tooling in [DEBUG.md](DEBUG.md).
...
See [Docker image publication and deployment](docs/docker-images.md) for registry login, SHA-tag publication, release aliases, local release checks, platform support and rollback. The release agent builds and publishes from the local machine after the final merge; GitHub Actions does not publish Docker images.
