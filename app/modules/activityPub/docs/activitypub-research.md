...
Local smoke status: `npm run activitypub:interop-smoke` now checks representative WebFinger, NodeInfo, actor, Note/Create, outbox filtering, content-type, sanitized rich-text, ActivityStreams tag, and attachment payloads without live network dependencies. `npm run activitypub:remote-server-smoke` is an operator-run live Fediverse smoke: it resolves a configurable WebFinger resource or direct actor URL, fetches a public Note from featured/outbox collections when available, and feeds a signed fixture-equivalent `Create(Note)` through GeeSome's existing shared-inbox storage/review/post-create path. `npm run activitypub:ownership-challenge-smoke` covers the signed migration ownership proof path: it skips without actor configuration, records actor public-key capability, prints the exact challenge body and proof shape when no signer is supplied, verifies the deterministic local harness signer, and can run an operator-provided signer command for compatible external tools. `npm run activitypub:bluesky-bridge-smoke` adds the Bluesky boundary check through Bridgy Fed: it discovers a bridged public Bluesky actor, converts a public ATProto feed item to ActivityPub when available, and feeds that same local harness path. `npm run bluesky:credentialed-smoke` adds optional native ATProto credentialed coverage; it skips without credentials, verifies account login/stored lookup plus public source preview, local source import, source refresh, and best-effort source sync when credentials exist, and requires `BLUESKY_CREDENTIAL_SMOKE_WRITE=1` before creating, updating, and deleting a temporary remote record. Smoke scripts can persist secret-free JSON reports with script-specific `*_SMOKE_REPORT_PATH` env vars or `GEESOME_SMOKE_REPORT_PATH`. Public Mastodon, Bridgy Fed, native ATProto, deterministic ownership-signer, and independent Fedify CLI read evidence is recorded in [live-interoperability.md](./live-interoperability.md). Credentialed writes and full public staging-node inbox/outbox exchange remain operator release gates because they need disposable accounts and a reachable GeeSome node.

ActivityPub source-feed UI status/backlog: geesome-ui now has a top-level ActivityPub Sources reader where operators can subscribe to remote ActivityPub actors and read cached posts in a chronological feed, with the first preset covering the official Bluesky account via Bridgy Fed. The backend exposes source-oriented APIs instead of requiring the UI to scrape admin remote-object review routes directly: resolve actor handle/URL, list/add/update/remove subscriptions, manually or asynchronously refresh public `featured`/`outbox` source objects into the local cache, enqueue stale active subscriptions through an opt-in poller, process queued refresh jobs through the shared async-operation queue/worker, store subscription read/refresh/error state, optionally request a federation-level outbound Follow from an explicit local group actor to the subscribed source actor, and list sanitized feed items with source actor metadata, canonical rich text or escaped text, bounded attachment metadata, `embedPolicy`, remote URLs, review/import state, and optional `(publishedAt, id)` cursor pagination that skips expensive totals. Source follow acceptance reuses the existing signed remote `Accept(Follow)` handling and group following collection. Native Bluesky/ATProto import/cross-posting is now tracked as the next bridge-free phase, through a dedicated Bluesky module or socNet import driver rather than the ActivityPub module; public read/projection smoke, credentialed local source-import/refresh/sync smoke, an admin preview API, the reusable social-import adapter, one-page async import, local source subscription CRUD, queued/polled one-page subscription refresh, and bounded imported-post update/delete sync exist, while broader live conformance and frontend native Bluesky feed UX remain future slices. The frontend implementation plan and dual-viewport e2e/screenshot coverage live in `geesome-ui/docs/activitypub-source-feed-plan.md`.
...
- Local deterministic interop smoke. Status: `npm run activitypub:interop-smoke` checks representative WebFinger, NodeInfo, actor, Note/Create, outbox, content-type, sanitized rich-text, ActivityStreams tag, and attachment payloads before remote-server smoke. Set `ACTIVITYPUB_INTEROP_SMOKE_REPORT_PATH` or `GEESOME_SMOKE_REPORT_PATH` to persist the JSON report.
- Live remote-server smoke. Status: `npm run activitypub:remote-server-smoke` resolves a configurable ActivityPub actor, fetches a public Note from featured/outbox collections when available, and verifies that a signed fixture-equivalent received Note reaches GeeSome's remote-object review/import path. It reports clear skips when an actor has no bridge/public Note available. Set `ACTIVITYPUB_REMOTE_SMOKE_REPORT_PATH` or `GEESOME_SMOKE_REPORT_PATH` to persist the JSON report.
- Live ownership-challenge smoke. Status: `npm run activitypub:ownership-challenge-smoke` creates the same short-lived migration ownership challenge used by the API, reports actor discovery and public-key capability, prints the exact HTTP-signature proof requirements when a signer is not supplied, verifies deterministic local harness proofs, and supports an operator `ACTIVITYPUB_OWNERSHIP_CHALLENGE_SMOKE_SIGN_COMMAND` for compatible ActivityPub tooling. Set `ACTIVITYPUB_OWNERSHIP_CHALLENGE_SMOKE_REPORT_PATH` or `GEESOME_SMOKE_REPORT_PATH` to persist the JSON report.
- Live Bluesky bridge smoke. Status: `npm run activitypub:bluesky-bridge-smoke` defaults to the official public `bsky.app` account, follows the Bridgy Fed ActivityPub discovery/conversion path, signs a fixture-equivalent `Create(Note)` with a disposable harness key for the bridged actor URL, and asserts local `ActivityPubRemoteActor`, `ActivityPubObject`, review, idempotency, and accepted-post creation behavior. Set `ACTIVITYPUB_BLUESKY_SMOKE_REPORT_PATH` or `GEESOME_SMOKE_REPORT_PATH` to persist the JSON report. This is a bridge-boundary smoke, not native ATProto import.
- Independent Fedify parsing. Status: `fedify lookup` successfully resolved and parsed the public Mastodon project actor on 2026-07-13; see [live-interoperability.md](./live-interoperability.md).
- Public staging federation. Status: `fedify inbox`, ActivityPub.Academy, or a disposable Mastodon-compatible actor remains an operator release gate for signed outbound delivery and inbound reply/update/delete exchange because the target GeeSome node must be publicly reachable.
- Bluesky compatibility smoke must be explicit about protocol boundaries. Bluesky uses AT Protocol, so ActivityPub exchange should be tested through a bridge such as Bridgy Fed: local GeeSome group post delivery should appear on the bridged Bluesky side, and a Bluesky reply/mention should come back as a signed ActivityPub inbox/shared-inbox activity that GeeSome stores as a remote object. The current `npm run activitypub:bluesky-bridge-smoke` covers public bridged-account discovery, public post conversion, and local shared-inbox storage/review/post-create behavior with a fixture-equivalent signature; it cleanly skips when the account is not bridge-enabled or no bridge-visible public Note is available. The native ATProto phase now also has `npm run bluesky:atproto-smoke`, which verifies unauthenticated public XRPC reads and canonical projection for configurable public handles without writing to the database, plus a read-only admin preview route for the same configured public-XRPC projection. `npm run bluesky:credentialed-smoke` is the credentialed counterpart: it seeds an in-memory `socNetAccount` through real module login, verifies DID/handle ownership, previews source identity, exercises local public-feed import/source-refresh/source-sync paths, and can opt into create/idempotency/update/delete plus image-upload-failure link fallback using non-production credentials. Next direct Bluesky data-exchange slices should focus on broader real feed/account migration jobs and conformance evidence rather than duplicating this smoke's import or cross-post lifecycle.
