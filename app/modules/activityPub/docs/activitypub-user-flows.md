...
6. User starts a migration job. Native Bluesky currently supports claimed imports after stored-account DID/handle proof: immediate imports process one page, and queued imports can walk bounded cursor pages through `maxPages`. It also has a bounded dry-run/apply reconciliation route for already-imported posts in one group, so stored Bluesky reply and quote metadata can be linked to local `replyToId` and `repostOfId` after matching target posts exist. ActivityPub currently supports a first conservative write job that caches eligible own-authored public objects as remote-object candidates through the async queue, with bounded collection-page walking through `maxPages`; visible GeeSome post creation still waits for ownership proof, moderation/review policy, UI controls, and ActivityPub-specific relation reconciliation.
...
10. If a remote placeholder later migrates to GeeSome, reconciliation links the placeholder to the new local group/account/posts by protocol identity so partial thread/history content becomes connected without duplicate posts. Native Bluesky already supports this for imported post reply/quote fields when both sides have matching AT URI source identity; ActivityPub still needs the equivalent visible-post reconciliation after proof/moderation makes imported posts visible.
...
- Simple remote social-page migration can import a user's public Bluesky or ActivityPub presence into a GeeSome personal group, preserve replies/reposts/quotes, create remote placeholders for referenced groups/accounts, and reconcile those placeholders when they later migrate. Native Bluesky imported-post reply/quote reconciliation is covered for already-imported group posts; ActivityPub visible-post reconciliation remains follow-up.
