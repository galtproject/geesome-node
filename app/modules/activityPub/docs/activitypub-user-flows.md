...
6. User starts a migration job. Native Bluesky currently supports claimed imports after stored-account DID/handle proof: immediate imports process one page, and queued imports can walk bounded cursor pages through `maxPages`. Full account-exhaustion controls and the equivalent ActivityPub write job remain follow-up work.
