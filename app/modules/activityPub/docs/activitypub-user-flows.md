...
6. User starts a migration job. Native Bluesky currently supports claimed imports after stored-account DID/handle proof: immediate imports process one page, and queued imports can walk bounded cursor pages through `maxPages`. ActivityPub currently supports a first conservative write job that caches eligible own-authored public objects as remote-object candidates through the async queue, with bounded collection-page walking through `maxPages`; visible GeeSome post creation still waits for ownership proof, moderation/review policy, UI controls, and a safe relation-reconciliation worker.
...
10. If a remote placeholder later migrates to GeeSome, reconciliation links the placeholder to the new local group/account/posts by protocol identity so partial thread/history content becomes connected without duplicate posts. Preview placeholders already expose the stable source-identity contract; the remaining work is a guarded job that rewrites local relations after both sides exist and source identity matches.
