...
6. User starts a migration job. Native Bluesky currently supports a claimed one-page async import after stored-account DID/handle proof, either immediately or through the persistent async-operation queue; the broader all-pages migration flow and the equivalent ActivityPub write job remain follow-up work.
