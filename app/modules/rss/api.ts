...
export default (_app: IGeesomeApp, rssModule: IGeesomeRssModule) => {
	 ...
	 * @apiQuery {Number} [limit=100] Maximum feed items to render, capped at 9999 for explicit archive reads.
	...
	_app.ms.api.onGet(rssModule.getRssGroupUrl(), async (req, res) => {
		...
		return res.send(await rssModule.groupRss((last(req.fullRoute.split('/')) as string).split('.')[0], host, null, {
