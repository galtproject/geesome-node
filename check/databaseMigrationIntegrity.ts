...
const coveredMigrations: CoveredMigration[] = [
  ...
  {
    module: 'database',
    file: '20260731000000-add-agent-api-metadata.cjs',
    verifies: ['storage-object SHA-256 metadata', 'API-key integration metadata', 'async-operation request correlation'],
  },
  {
    module: 'group',
...
const expectedColumns: ExpectedColumn[] = [
  ...
  {table: 'storageObjects', columns: ['sha256'], type: 'character varying'},
  {table: 'userApiKeys', columns: ['lastUsedAt'], type: 'timestamp with time zone'},
  {table: 'userApiKeys', columns: ['scopes'], type: 'text'},
  {table: 'userAsyncOperations', columns: ['requestId'], type: 'character varying'},
...
const expectedIndexes: ExpectedIndex[] = [
  ...
  {name: 'storage_objects_sha256_idx', table: 'storageObjects', columns: ['sha256', 'id']},
  {name: 'assets_user_storage_unique', table: 'assets', columns: ['userId', 'storageId'], unique: true},
  {name: 'assets_user_sha_idx', table: 'assets', columns: ['userId', 'sha256', 'id']},
  {name: 'asset_idempotency_user_namespace_key_unique', table: 'assetIdempotencyKeys', columns: ['userId', 'namespace', 'key'], unique: true},
  {name: 'asset_batches_user_key_unique', table: 'assetBatches', columns: ['userId', 'idempotencyKey'], unique: true},
  {name: 'asset_batch_items_batch_logical_unique', table: 'assetBatchItems', columns: ['assetBatchId', 'logicalId'], unique: true},
  ...
  {name: 'auto_action_dedupe_keys_user_identity_unique', table: 'autoActionDedupeKeys', columns: ['userId', 'identityKey'], unique: true},
  {name: 'auto_action_dedupe_keys_action_idx', table: 'autoActionDedupeKeys', columns: ['autoActionId']},
  {name: 'auto_action_dedupe_keys_updated_idx', table: 'autoActionDedupeKeys', columns: ['updatedAt', 'id']},
...
const countChecks: CountCheck[] = [
  ...
  duplicateCheck('asset owner storage duplicates', 'assets', ['userId', 'storageId']),
  duplicateCheck('asset idempotency duplicates', 'assetIdempotencyKeys', ['userId', 'namespace', 'key']),
  duplicateCheck('asset batch idempotency duplicates', 'assetBatches', ['userId', 'idempotencyKey']),
  duplicateCheck('asset batch logical item duplicates', 'assetBatchItems', ['assetBatchId', 'logicalId']),
  {
