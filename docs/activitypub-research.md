...
ActivityPub source-feed UI backlog: geesome-ui should add a top-level ActivityPub Sources reader where operators can subscribe to remote ActivityPub actors and read new posts in a chronological feed, with the first preset covering the official Bluesky account via Bridgy Fed. The backend should expose source-oriented APIs instead of requiring the UI to scrape admin remote-object review routes directly: resolve actor handle/URL, list/add/refresh/remove subscriptions, store subscription state/unread counters/errors, and list cursor-paginated sanitized feed items with source actor metadata, canonical rich text or escaped text, bounded attachment metadata, `embedPolicy`, remote URLs, and review/import state. The frontend implementation plan and required dual-viewport e2e/screenshot coverage live in `geesome-ui/docs/activitypub-source-feed-plan.md`.
