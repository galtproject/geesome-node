...
- `ActivityPubObject`: local post id or remote object URL, activity id, object id, type, origin, visibility, published time, and raw JSON. Status: local public group post Note/Create IDs are recorded idempotently when outbox or post-object serializers run, and signed shared-inbox `Create(Note)` replies to known local objects are recorded idempotently as remote rows. Creating GeeSome posts from remote objects and mapping delete/update activity remain future work.
...
- Handle `Delete`, non-follow `Undo`, `Block`, and `Flag`.
