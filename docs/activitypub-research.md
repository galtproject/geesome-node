...
- Define database records for actor keypairs, remote actors, follows, ActivityPub objects, and delivery attempts. Status: local actor keypair records, remote actor key/inbox cache records, inbound follow-state records, local and remote object/mention records, queued delivery records, bounded delivery retry processor, opt-in claim-backed delivery worker, and moderation-gated GeeSome post creation from accepted remote objects exist; automated remote post update/delete sync remains future work.
