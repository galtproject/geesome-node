...
- `Create` with `Note` and other supported review object types: store remote replies/mentions for moderation review, with native post creation still limited to accepted public Notes.
- `Delete`: tombstone cached remote objects.

Keep moderation-sensitive activities behind explicit storage/review boundaries: `Flag` stores pending reports, `Block` cancels follower delivery state, and remote `Create`/`Update` objects stay cached for review until a supported manual import path exists.
...
Remote `Create` replies and mentions are stored as `ActivityPubObject` records when they target a known local ActivityPub object or actor and use one of the supported review object types: `Note`, `Article`, `Page`, `Image`, `Video`, `Audio`, `Document`, `Question`, or `Event`. Creating visible GeeSome posts from those records is manual and moderation-gated: only an accepted public `Note` with sanitized canonical rich text can become a native remote post. Non-Note objects remain review/audit records until richer native object policies exist. Remote attachments default to provenance-only; preview/draft payloads expose bounded sanitized remote URL/media metadata such as media category, alt text, dimensions, duration, blurhash, and sensitive flag, plus an explicit `attachmentImportPolicy` showing the default mode and opt-in backup mode. The initial manual import creates `Post` rows with:
...
- bounded sanitized remote attachment/media metadata in `propertiesJson.activityPub.attachments`, `propertiesJson.activityPub.attachmentImportPolicy`, and `propertiesJson.activityPub.attachmentImportMode`; when AdminAll requests `importRemoteAttachments`, supported HTTP(S) media/document attachments are also backed up into GeeSome content storage and recorded in `propertiesJson.activityPub.attachmentBackups`

Before any remote ActivityPub object becomes a visible GeeSome/webview post, review the existing post rendering path because post bodies are HTML. Store the remote raw object for audit, but render only sanitized/escaped content with an explicit allowlist for tags, attributes, links, media embeds, and IPFS/IPNS URLs. Static-site generated post text, list title/description HTML, content-list text, header/footer HTML, generated title/meta headers, and admin remote-object API preview fields now have conservative sanitizer/escaping coverage with XSS fixtures; the admin post-draft route reports sanitized rich-text readiness only for accepted, not-yet-linked remote objects plus resolved local reply targets, bounded sanitized remote attachment/media metadata, and explicit remote attachment import policy, and the manual create route writes that canonical rich-text payload as native post content while optionally backing up supported HTTP(S) media/document attachments when requested. Signed remote tombstones now soft-delete linked imported posts when their source identity still matches. Frontend/admin UI adoption of the preview fields, richer backup/embed policy, and direct webview surfaces still need explicit review.

Attachments are represented first as remote URLs in `propertiesJson` with `attachmentImportPolicy.mode = 'provenanceOnly'`; AdminAll post creation can opt into HTTP(S) media/document backup, while IPFS/IPNS, link-only attachments, async retry policy, and richer embed rendering remain follow-up policy work.
