...
- bounded sanitized remote attachment/media metadata plus backup eligibility in `propertiesJson.activityPub.attachments`, `propertiesJson.activityPub.attachmentImportPolicy`, and `propertiesJson.activityPub.attachmentImportMode`; when AdminAll requests `importRemoteAttachments`, supported HTTP(S) media/document attachments are also backed up into GeeSome content storage and recorded in `propertiesJson.activityPub.attachmentBackups`
...
Attachments are represented first as remote URLs in `propertiesJson` with `attachmentImportPolicy.mode = 'provenanceOnly'`; each attachment now exposes whether `backupOnCreate` can fetch remote bytes, with unsupported category or URL-scheme reasons for provenance-only entries. AdminAll post creation can opt into HTTP(S) media/document backup, while IPFS/IPNS byte import, link preview/embed rendering, and async retry policy remain follow-up policy work.
