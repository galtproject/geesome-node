...
- `ActivityPubObject`: local post id or remote object URL, activity id, object id, type, origin, visibility, published time, and raw JSON. Status: local public group post Note/Create IDs are recorded idempotently when outbox or post-object serializers run, signed shared-inbox `Create(Note)` replies to known local objects are recorded idempotently as remote rows, and signed shared-inbox `Delete` tombstones cached remote rows from the same actor. Creating GeeSome posts from remote objects and mapping update activity remain future work.
...
- Handle non-follow `Undo`, `Block`, and `Flag`.
