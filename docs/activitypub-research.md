...
- Process remote `Create(Note)` replies/mentions. Status: signed replies to known local objects and mentions of known local group actors are stored idempotently as `ActivityPubObject` rows and exposed through AdminRead remote-object list/detail routes with raw parsed objects for audit plus sanitized preview fields and canonical rich-text content previews for admin review/import planning; moderation and GeeSome post creation remain future work.
