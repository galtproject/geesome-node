...
- Inbound ActivityPub/Matrix HTML: sanitize, normalize, and parse into canonical rich text before it can become a native editable GeeSome post. The original remote object may be stored for audit/debug, but it must not be rendered directly. Status: cached remote ActivityPub object previews expose sanitized `contentHtml`, plain `contentText`, canonical `contentRichText` converted from the sanitized content, read-only post-draft metadata, and persistent review-state decisions for review/import follow-ups.
...
- Add moderation controls. Status: the admin post-draft projection now reports whether a cached remote object is a public `Note` with sanitized rich-text content ready for future import, and AdminAll users can persist pending/accepted/rejected review decisions; post create/update/delete actions remain future work.
