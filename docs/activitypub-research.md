...
Delivery claims mirror the auto-action claim pattern: fresh model-sync tables include the claim columns/index, upgraded tables get them from the migration, and new runtime code omits claim attributes until a migrated restart makes them available. With claims active, due delivery selection uses `FOR UPDATE SKIP LOCKED`; without them, the processor keeps the previous bounded fallback path.
...
- Define database records for actor keypairs, remote actors, follows, and delivery attempts. Status: local actor keypair records, remote actor key/inbox cache records, inbound follow-state records, queued delivery records, bounded delivery retry processor, and opt-in claim-backed delivery worker exist; object records remain future work.
