...
ActivityPub source-feed UI status/backlog: geesome-ui now has a top-level ActivityPub Sources reader where operators can subscribe to remote ActivityPub actors and read cached posts in a chronological feed, with the first preset covering the official Bluesky account via Bridgy Fed. The backend exposes source-oriented APIs instead of requiring the UI to scrape admin remote-object review routes directly: resolve actor handle/URL, list/add/update/remove subscriptions, manually refresh public `featured`/`outbox` source objects into the local cache, store subscription read/refresh/error state, and list sanitized feed items with source actor metadata, canonical rich text or escaped text, bounded attachment metadata, `embedPolicy`, remote URLs, and review/import state. Remaining source-feed work is background-safe polling/follow behavior, cursor pagination if offset lists become too costly, and direct Bluesky/ATProto import/cross-posting as a separate module or socNet import driver. The frontend implementation plan and dual-viewport e2e/screenshot coverage live in `geesome-ui/docs/activitypub-source-feed-plan.md`.
