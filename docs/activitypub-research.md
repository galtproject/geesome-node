...
- Expose following collection. Status: public following route lists accepted outbound follows; pending outbound follows stay hidden until a later `Accept` is recorded.
- Handle inbound `Accept`/`Reject` for outbound follows.
