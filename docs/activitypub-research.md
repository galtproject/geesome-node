...
- Process remote `Create(Note)` replies/mentions. Status: signed replies to known local objects are stored idempotently as `ActivityPubObject` rows and exposed through an AdminRead remote-object list; moderation and GeeSome post creation remain future work.
