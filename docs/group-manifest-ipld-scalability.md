...
2. Keep the default-on `group-derived-state` queue under restored-data coverage. `database:derived-state-async-rehearsal` passed the May 15 posts dump with `GROUP_DERIVED_STATE_ASYNC=1`; rerun it on larger restored production-shaped data when available. The queue records post/group manifest and static-directory checkpoint status, uses attempt-aware failure messages, processes bounded kick batches, can run through an interval worker, and can still be disabled with `GROUP_DERIVED_STATE_ASYNC=0` if rollout evidence exposes a regression.
3. Keep the manual `database:derived-state-integrity` verifier as the operator safety net.

Later protocol work:

1. Keep public shared serving/header metadata, canonical local pin state, and nullable ownerless/federated identity metadata on the `StorageObject` helper with deterministic `Content` fallback for old rows and restored-data repair, then wire fuller producer-specific policy onto the same registry once restored-data coverage proves the registry is complete enough.
