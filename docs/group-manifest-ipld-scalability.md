...
2. Promote the opt-in `group-derived-state` queue into the default write path after the default-on rollout decision. `database:derived-state-async-rehearsal` passed the May 15 posts dump with `GROUP_DERIVED_STATE_ASYNC=1`; rerun it on larger restored production-shaped data when available. The queue already records post/group manifest and static-directory checkpoint status, uses attempt-aware failure messages, processes bounded kick batches, and can run through an interval worker.
