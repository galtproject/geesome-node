...
2. Promote the opt-in `group-derived-state` queue into the default write path once `database:derived-state-async-rehearsal` passes on restored production-shaped data with `GROUP_DERIVED_STATE_ASYNC=1`. The queue already records post/group manifest and static-directory checkpoint status, uses attempt-aware failure messages, processes bounded kick batches, and can run through an interval worker.
