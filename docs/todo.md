...
- Native Bluesky/ATProto smoke and tests: add a public unauthenticated XRPC smoke for a configurable account, a seeded `socNetAccount` harness for credentialed import/cross-post paths, and unit tests for ATProto record/facet/embed/delete/update mapping into canonical rich text, source identity, review/import state, and idempotent GeeSome group/post storage.
