...
1. Add credential and pin-health operator flows after reconciliation is stable. Provide a secret-safe credential test, last successful check/error summaries, stale/failed counts, retry/reconcile actions, and bounded history. The UI should distinguish queued, provider-accepted, confirmed, stale, and failed states instead of presenting one optimistic pinned flag.
