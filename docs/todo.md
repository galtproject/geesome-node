...
- Database scalability and data-model lifecycle: [#880](https://github.com/galtproject/geesome-node/issues/880) for large groups with hundreds of thousands of posts and attached content records. Status: first review and deterministic inventory are covered by `docs/database-scalability-review.md` and `npm run database:scalability`; follow-up implementation should add transactional post creation/local ID allocation, remote manifest import tests, measured index migrations, async manifest rebuild jobs, cursor pagination, and large-group Docker benchmarks.
