...
Status: completed. The original 2026-07-14 Docker run completed with `431 passing, 11 pending`, but late Telegram-import tests coincided with repeated `SequelizeConnectionError: sorry, too many clients already` errors from the auto-action worker. Attribution found that every test app opened the shared Sequelize pool but `app.stop()` never closed it, while interval workers were not consistently owned or drained by their modules. App shutdown now closes gateway/API ingress first, drains modules in dependency-safe order, and closes the database pool last. A shared interval-worker helper prevents overlapping ticks, unreferences timers, tracks immediate runs, and makes shutdown await active callbacks. Auto-actions, ActivityPub, Bluesky, group derived-state, and storage-space sampling/removal workers now use module-owned lifecycle handles. Async-operation queue processing locks are app-instance-local, reject new work after stop, and drain active durable queue processors before database closure. The executable entrypoint routes `SIGTERM`/`SIGINT` through a bounded idempotent shutdown controller; repeated signals force a conventional signal exit, and Docker Compose grants more time than the application deadline. Storage and communicator stop paths are idempotent and ownership-aware, the shared profiler uses per-app lifecycle handles and drains pending writes, and a sequential same-port app restart proves that ingress and database resources can be reopened after teardown. Dedicated connection diagnostics are disabled by default and expose only configured limits, scalar Sequelize pool gauges, and bounded aggregate PostgreSQL activity groups under an explicit flag or exact debug namespace. Startup reads PostgreSQL capacity once, caches the deployment budget, and emits one scalar warning when configured process pools plus auxiliary clients cannot preserve the operator reserve. Configured modules now initialize all-or-nothing: failure preserves and annotates the original error, stops initialized modules in lifecycle order, and closes model-initialization Sequelize instances or listening HTTP servers that are not yet attached to the app. The final full Docker run completed with `465 passing, 11 pending` and no PostgreSQL client-exhaustion, post-shutdown database errors, cleanup warnings, or unsolicited diagnostic output.
...
- Keep process signal registration in the executable entrypoint rather than reusable app/module factories. `SIGTERM`/`SIGINT` must stop accepting work before draining servers/workers, close storage/communication resources, and close Sequelize last; Docker's stop grace period must remain longer than the application deadline.
- API/gateway server, storage, communicator, and memory-profiler stop methods are awaitable and idempotent; preserve that contract. Embedded Helia belongs to the app, Kubo RPC is externally owned unless `STORAGE_STOP_REMOTE_NODE=1`, Fluence stops only an initialized client, and profiler state remains active until every live app owner releases it. Repeated app construction must not accumulate listeners, timers, pending file writes, or resource handles.
...
2. Completed 2026-07-15: the executable entrypoint owns `SIGTERM`/`SIGINT`, closes gateway/API ingress first, drains idempotent `app.stop()` within `GEESOME_SHUTDOWN_TIMEOUT_MS`, uses meaningful clean/error/forced exit codes, and has a real child-process signal regression with an active HTTP request. Compose exposes a longer configurable stop grace period.
3. Completed 2026-07-15: storage and communicator stop paths are ownership-aware and idempotent; shared profiler timers, event-loop histograms, and pending file writes are reference-counted across app instances and drained before database closure; the stop order keeps database last even when earlier configured modules exist; and a same-port sequential app restart verifies closed Sequelize pools and released ingress handles. The focused profiler/process lifecycle tests passed with `12 passing`; an isolated render lifecycle and the combined app-plus-render sequence passed after one transient first full-suite render setup failure; the repeated full Docker suite passed with `449 passing, 11 pending` and no post-close database activity.
4. Completed 2026-07-15: `GEESOME_DATABASE_CONNECTION_DIAGNOSTICS=1` or the exact `geesome:database:connections` namespace starts one non-overlapping database-owned sampler. It reports configured pool limits, scalar size/available/borrowed/waiting gauges, and at most 20 aggregate `pg_stat_activity` application/state groups; broad debug wildcards remain inert, the interval and activity count are clamped, active sampling drains before Sequelize closes, and `DATABASE_APPLICATION_NAME` makes process roles attributable. Unit and lifecycle coverage passed, and the full Docker suite completed with `454 passing, 11 pending`.
5. Completed 2026-07-15: the production formula now accounts for every GeeSome process pool, auxiliary clients, and an explicit PostgreSQL reserve. Startup reads `max_connections` once, caches the calculated budget for inspection, and emits one scalar warning only when the configured deployment cannot fit. Restricted environments can disable the read; failures remain non-fatal, and no request-time query, automatic pool resize, or server reconfiguration was added. Unit, app integration, and the full Docker suite passed with `460 passing, 11 pending`.
6. Completed 2026-07-15: configured module failures are fatal and carry `bootstrapStage`/`bootstrapModuleName` metadata on the preserved original error. Already-initialized modules stop in ingress-first, database-last lifecycle order; cleanup failures remain bounded scalar warnings and cannot mask the startup cause. Sequelize closes when model loading fails, API/gateway servers close when route registration fails after listening, and per-app/database config merges no longer mutate imported defaults. Focused bootstrap, shutdown, and gateway tests passed with `13 passing`; the full Docker suite passed with `465 passing, 11 pending`.
...
Status: complete for the Pinata phase on `dev`. Manual and automatic pinning, explicit group-post target policy, user and group account UI, durable reconciliation, operator health and recovery controls, persistent pending-job deduplication, bounded cross-process policy-cache freshness, execution-time obsolete-job cancellation, bounded provider-call safety, and the shared dedupe lifecycle are shipped. Additional providers and release-bound production schema preparation remain separate follow-up work.
...
- Completed 2026-07-16: automatic jobs now reload current policy before provider access and return structured terminal skips for deleted/disabled/moved accounts, removed group targets, missing content, and unpublished/ineligible posts. Manual pin errors remain unchanged. Local disable/scope-change/delete paths atomically deactivate active jobs by account identity prefix and clear execution claims; cross-process races remain protected by execution preflight. Group scope later became authoritative across creator/admin rotation. Prefix isolation, terminal-skip/provider-call, legacy action, and local cancellation coverage pass; the full Docker suite completed at that point with `470 passing`, `11 pending`.
- Completed 2026-07-17: bound Pinata calls with a configurable 1–120 second timeout, disabled redirects, bounded credential-redacted error details, retryable-versus-terminal classification, and module-shutdown cancellation. The canonical Pinata endpoint remains zero-configuration. Custom endpoints now require an explicit enable flag and exact host allowlist, HTTPS, no URL credentials, all-public DNS answers, and DNS-pinned connections so credentials are rejected before SSRF, redirect, or rebinding paths. Generic auto actions deactivate explicitly terminal failures without spending their remaining retry budget. Focused provider tests pass and the full Docker suite completes with `478 passing`, `11 pending`.
- Completed 2026-07-17: made unique auto-action chains transactional. A winning dedupe root now creates or resolves ordered children, attaches pivots, and updates its dedupe pointer in one transaction; concurrent losers create no candidate children, and attachment failure rolls the whole graph back. Existing children can still be referenced by ID, while inline child definitions avoid caller-created orphans. Cron-driven cleanup removes only old inactive or orphaned keys in configurable bounded batches, preserves active/recent identities, and cannot block due jobs when maintenance fails. Concurrent-chain, rollback, cleanup, and cron-failure coverage pass; the full Docker suite completes with `482 passing`, `11 pending`.
- Completed 2026-07-17: defined pin-account scope and complete discovery. Direct-user lookups require `groupId IS NULL`; group-scoped credentials are controlled by current group editors and remain operational across creator/admin rotation, while creator `userId` is retained only for audit and auto-action attribution. Scope changes require replacement accounts, deletion retains historical ledger rows without remote unpin, and automatic user/group policy discovery now walks stable `(name, id)` cursor batches plus an id-ordered legacy unnamed phase instead of silently stopping at 100 rows. Ownership and 106-account-per-scope traversal coverage pass; the full Docker suite completes with `484 passing`, `11 pending`.
- Completed 2026-07-17: changed `PinStorageObject` from an optimistic success marker into a per-account attempt state machine with requested, provider-accepted, confirmed, missing, retryable-failure, and terminal-failure states. Attempts snapshot account scope, keep bounded provider result/error details and timestamps, and use attempt tokens so late concurrent completions cannot overwrite newer state. Every authorized target is recorded, including group post manifests without `Content` rows. Provider acceptance no longer sets local aggregate pin booleans; only confirmed/legacy-confirmed claims appear as remotely pinned in storage-space, while uncertain nonterminal states still fail closed for deletion. Account deletion retains ledger history.
- Completed 2026-07-18: added the first durable provider-reconciliation path. Pinata inspection checks `data/pinList` before `pinning/pinJobs`, preserves active retrieval as accepted, confirms pinned rows, distinguishes known terminal jobs, and marks missing only when both sources are empty. Due rows are selected in bounded per-account batches and queued through shared async operations. PostgreSQL advisory-lock transactions enforce the account concurrency cap and atomically claim individual ledger rows with expiry, while retryable failures use bounded exponential backoff. A warm persisted-dev-table test also proved additive no-drop model sync before release migrations exist. Periodic scheduling remains intentionally disabled until the queue contract is reviewed.
- Completed 2026-07-18: wrapped provider reconciliation in an opt-in interval worker. `PIN_RECONCILIATION_WORKER=1` performs an immediate restart sweep and bounded periodic queue/process passes with configurable interval, batch, per-account, and claim-TTL controls. Sweeps do not overlap in one process, shutdown stops new ticks before aborting requests and draining current work, and claim-time due checks make concurrently duplicated startup jobs harmless after another worker advances the ledger state. Existing queued work is still processed when due-row discovery fails; the full Docker suite passes (`507 passing`, `11 pending`).
- Completed 2026-07-18: defined canonical pin provenance without another persisted aggregate. Current local writes use `StorageObject.isPinned`, historical storage/content flags remain an explicit `localOrLegacy` compatibility source, and per-account `PinStorageObject` rows independently derive confirmed remote availability and broader fail-closed deletion protection. Reference counting now exposes the provenance snapshot while preserving its existing blocker fields. Shared-storage coverage proves that one account becoming missing cannot erase another confirmed account claim or an intentional local pin, and that provider-accepted claims protect deletion without claiming confirmed availability; the full Docker suite passes (`508 passing`, `11 pending`).
- Completed 2026-07-18: added the backend pin-account operator contract. Authorized account owners and current group editors can test canonical Pinata credentials through a read-only endpoint without exposing secrets or provider response bodies, inspect bounded ledger health/history with requested, accepted, confirmed, missing, retryable-failure, and terminal-failure states kept distinct, and queue bounded account-wide or exact-storage reconciliation retries. Account-wide runs rotate through least-recently-updated rows using an account/history index instead of starving entries beyond the first batch. Custom endpoints and unsupported providers fail closed until they have explicit credential/status adapters; manual reconciliation reuses the durable shared queue and never implies remote unpin. The full Docker suite passes (`515 passing`, `11 pending`), followed by a focused model-sync/PostgreSQL rerun for the final index (`7 passing`).
- Completed 2026-07-20: shipped the pin-account operator UI through `geesome-ui` [#34](https://github.com/galtproject/geesome-ui/pull/34) at commit `78421dc`. Personal and group account surfaces show primary health and recovery actions first, keep bounded ledger diagnostics/history secondary, test credentials without exposing secrets, and support account-wide reconciliation plus exact failed-entry retries. Health requests use bounded client concurrency, unsupported providers remain fail-closed, and responsive Node 18 production-build plus 11-scenario mobile/desktop E2E screenshot coverage passes.

Recent findings to preserve:

- Cache snapshots are discovery hints only. Job execution reloads the account by immutable ID and rechecks direct ownership or the saved group scope plus exact eligible post target from the database; do not turn cached policy into authorization data.
- Automatic policy is revalidated at execution and local account changes cancel matching pending identities promptly. Keep cached discovery non-authoritative, preserve terminal skip results as successful one-shot completion, and do not turn obsolete policy/content state into provider retries.
...
- Old `Content.isPinned` and synchronized `StorageObject.isPinned` values may still include remote-derived history. They remain a named `localOrLegacy` compatibility source instead of being rewritten or cleared; resolving their exact historical origin requires a release-bound audit/backfill policy.
- Group-account creator `userId` still owns the durable auto-action row for attribution, even though it grants no credential access and automatic group execution follows the saved group policy across admin rotation. A future worker/account-principal model can remove this attribution mismatch without changing authorization.

Optional follow-up slices:

1. Add explicit status adapters before reconciling custom pin endpoints or additional providers; unsupported adapters must remain fail-closed and must not turn uncertainty into `missing`.
2. Prepare release-bound schema upgrades only when promoting `dev`: add migrations and migration-integrity checks for existing production tables, constraints, indexes, state backfills, and dedupe cleanup. Keep brand-new unreleased tables model-sync-only until that release boundary.
...
- Policy-race tests queue work, then disable/delete the account or remove the selected group target before execution; obsolete automatic jobs must stop without calling the provider, while group policies remain valid across creator/admin rotation and manual operations retain their documented scope authorization.
...
- Ownership tests cover creator removal, group admin changes, account deletion with retained history, more than 100 configured user and group accounts, and exact post-attachment authorization. Ledger-state coverage must additionally keep the same `storageId` independent across users/providers.
