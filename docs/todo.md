...
- Add a global UI menu item and screen for storage-space analysis: show which file catalogs and groups use how much space, then let operators drill into largest files, file types, duplicate/shared storage IDs, and cleanup candidates. Status: added as a dedicated planning slice; first `geesome-node` aggregate helpers live in a dedicated `storageSpace` module with AdminRead API routes, cached snapshot routes/table, queued async refresh path, file-catalog root/folder drilldown, group-post drilldown, and the `geesome-ui` global screen. Remaining implementation should coordinate generated-output DAG accounting, restored-data measurement, finer per-query progress, and deeper cleanup/detail views.
...
Status: in progress, with the first backend aggregate/API seams, cached snapshot routes/table, queued async refresh path, file-catalog root/folder drilldown endpoint, group-post drilldown endpoint, and `geesome-ui` global navigation/screen in place. The dedicated `storageSpace` module reports logical content bytes separately from deduplicated physical storage bytes for overview, type breakdown, top content, top file-catalog file/folder, top group, and group-post views; AdminRead routes expose live results under `admin/storage-space/*`; and a model-sync `StorageSpaceSnapshot` table can store timestamped analyzer snapshots for operator refreshes. The snapshot refresh now uses the shared async-operation queue runner under the `storage-space-snapshot` module for progress polling, while the database module keeps only the synced snapshot model and shared SQL connection. Remaining work is generated-output DAG accounting, restored-data measurement, finer per-query progress, and deeper cleanup/detail views.
