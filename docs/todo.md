...
- Add a dedicated Bluesky/ATProto module or socNet import driver, not ActivityPub-specific code, because Bluesky uses AT Protocol records and XRPC APIs rather than signed ActivityPub inbox activities. Status: first native helpers and read-only module/API live under `app/modules/bluesky`, but they intentionally stop at public read/projection and do not create social-import channels or posts yet.
