...
- GeeSome does not yet expose a public callback route that receives a remote actor's challenge `POST` directly; current verification is an API-submitted proof object.
- Expired and consumed challenges are rejected, but there is not yet a dedicated cleanup/retention pass for old challenge rows.
...
- Decide whether the product flow should stay detached-proof-only for advanced/operator tooling, add a public proof receiver/normalizer for compatible ActivityPub tools, or support both. Document the chosen request method, URL, headers, digest/body, and key-ownership expectations.
- Add bounded cleanup for expired/consumed challenge rows and request-level abuse controls for challenge creation/verification by user, actor, and/or IP using existing rate-limit helpers where they fit.
...
- Focused tests for cleanup/retention and abuse-limit behavior once those controls are added.
...
- Signed ActivityPub ownership challenges have deterministic coverage, but no live actor/tool compatibility smoke yet.
...
- ActivityPub live smoke should cover both ordinary federation behavior and the ownership-challenge proof path when a compatible remote/tool can sign the exact challenge request; otherwise it should skip with a clear reason.
- Fedify CLI, ActivityPub.Academy, or a disposable Mastodon-compatible actor should be used to check discovery, actor/outbox/object payloads, inbox/shared-inbox signatures, follow accept/delivery, create/update/delete/tombstone behavior, and conformance diagnostics without becoming a default CI dependency.

Verification:

- New smoke command(s) with documented env vars and clear skip/failure output.
- Credentialed Bluesky smoke should cover account verification, import/source refresh, cross-post create/update/delete, media fallback policy, idempotency, and source identity without exposing secrets.
- ActivityPub live smoke should record the remote/tool capabilities it tested, including whether signed ownership proof was supported or skipped.
