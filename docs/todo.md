...
- Add a dedicated Bluesky/ATProto module or socNet import driver, not ActivityPub-specific code, because Bluesky uses AT Protocol records and XRPC APIs rather than signed ActivityPub inbox activities. Status: native helpers, read-only module/API, a reusable `BlueskyImportClient`, and a first async import route live under `app/modules/bluesky`; the route creates/reuses a social-import channel/group from author metadata and imports one public feed page, but no worker yet creates persistent Bluesky source subscriptions or long-running poll/sync state.
