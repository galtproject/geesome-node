...
- Production high/critical alerts are cleared after [#840](https://github.com/galtproject/geesome-node/issues/840). Remaining all-dependency high alerts are dev-only `minimatch` chains in API-doc tooling (`apidoc`, `apidoc-plugin-ts`, and their old glob/globby paths), so they should be handled in the API-doc tooling track with generation smoke tests instead of forced into the runtime dependency batch. The deprecated `geesome-libs -> geesome-wallet-client -> web3-provider-engine -> request` stack remains a moderate follow-up migration target.
