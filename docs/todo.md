...
Status: in progress, with the first backend aggregate/API seams, cached snapshot routes/table, queued async refresh path, and `geesome-ui` global navigation/screen in place. `database.getStorageSpaceOverview`, type breakdown, top content, top file-catalog file, and top group helpers now report logical content bytes separately from deduplicated physical storage bytes; AdminRead routes expose live results under `admin/storage-space/*`; and a model-sync `StorageSpaceSnapshot` table can store timestamped analyzer snapshots for operator refreshes. The snapshot refresh can now run through `UserOperationQueue` / `UserAsyncOperation` under the `storage-space-snapshot` module for progress polling instead of blocking the request. Remaining work is generated-output DAG accounting, restored-data measurement, finer per-query progress, and deeper folder/root drilldowns.
