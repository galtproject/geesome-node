...
- Add a dedicated Bluesky/ATProto module or socNet import driver, not ActivityPub-specific code, because Bluesky uses AT Protocol records and XRPC APIs rather than signed ActivityPub inbox activities. Status: native helpers, read-only module/API, and a reusable `BlueskyImportClient` live under `app/modules/bluesky`; the adapter can feed projected posts into `socNetImport`, but no admin route or worker yet creates persistent Bluesky source subscriptions, social-import channels, or posts.
