...
- Store inbound follows/accepts and minimal remote actor metadata. Remote actor key/inbox metadata is now cached for signature verification; follow records and accept/reject activity handling are still future work.
...
- ActivityPub-specific Sequelize models for actor, remote actor, follow, object, and delivery records. Model-sync `ActivityPubActor` stores local group actor URLs and encrypted signing keys, and `ActivityPubRemoteActor` stores fetched remote actor URL/key/inbox metadata; follows, objects, and delivery rows are still future slices.
