...
- API/gateway server stop methods are awaitable and idempotent; preserve that contract. Storage, communicator, and the global memory profiler still need explicit lifecycle verification. Repeated app construction must not accumulate listeners, timers, or resource handles.
...
3. Complete storage/communicator/memory-profiler shutdown ownership and add a repeated app start/stop test that checks active handles/listeners and proves no callback reaches Sequelize after database closure. API/gateway closure became awaitable and idempotent in slice 2.
