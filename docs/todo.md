...
ActivityPub source-feed direction: add a user-facing geesome-ui ActivityPub Sources section where operators can subscribe to remote ActivityPub actors and read new posts in a feed-style view. The first preset should support the official Bluesky account through Bridgy Fed (`acct:bsky.app@bsky.brid.gy`) while keeping direct ATProto import/cross-posting separate. Backend work should add source-oriented APIs instead of making the UI scrape admin review routes directly: resolve source handle/actor URL, list/add/refresh/remove subscriptions, expose subscription state/unread counters/errors, and list sanitized feed items with cursor pagination, source actor metadata, `contentText`/canonical rich text, bounded attachment metadata, `embedPolicy`, remote URLs, and local review/import state. Frontend implementation detail lives in `geesome-ui/docs/activitypub-source-feed-plan.md` and must include a dual-viewport Playwright e2e/screenshot test for source subscription, Bluesky-through-Bridgy preset selection, feed rendering, refresh/unsubscribe/error states, and unsafe HTML escaping.
...
- Add user-facing source subscription/feed support. Backend source APIs should resolve ActivityPub actors/handles, store subscription state independently from review/import decisions, refresh/poll/follow sources safely, and return cursor-paginated sanitized feed items suitable for the geesome-ui ActivityPub Sources reader.
...
- geesome-ui ActivityPub Sources e2e with mocked `$geesome` APIs: subscribe to the `@bsky.app via Bridgy Fed` preset, render a source feed with safe rich text, links, attachment `embedPolicy`, unread/error states, and verify mobile/desktop screenshot sidecars for no broken images or horizontal overflow.
