...
- The deterministic and live smoke scripts print secret-free JSON reports and can persist the same payload with a script-specific `*_SMOKE_REPORT_PATH` env var or the shared `GEESOME_SMOKE_REPORT_PATH` env var. Real hosted-account/tool compatibility evidence still needs operator runs.
