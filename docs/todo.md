...
- Remaining high/critical alerts need larger migrations rather than direct lockfile bumps: old Telegram bot `request` chains, the `telegram -> socks -> ip` chain, and old `geesome-libs` wallet/libp2p crypto dependencies. The old `apidoc`/`apidoc-core` chain was removed separately in [#802](https://github.com/galtproject/geesome-node/issues/802).
