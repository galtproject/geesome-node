...
- Add a global UI menu item and screen for storage-space analysis: show which file catalogs and groups use how much space, then let operators drill into largest files, file types, duplicate/shared storage IDs, and cleanup candidates. Status: added as a dedicated planning slice; first `geesome-node` aggregate helpers live in a dedicated `storageSpace` module with AdminRead API routes, cached snapshot routes/table, queued async refresh path, file-catalog root/folder drilldown, group-post drilldown, generated/static output source accounting, and the `geesome-ui` global screen. Remaining implementation should coordinate true IPFS DAG byte traversal for unknown generated refs, restored-data measurement, finer per-query progress, and deeper cleanup/detail views.
...
- Storage space analysis UI: add a global navigation entry and operator screen for file-catalog/group space usage, largest files, MIME/file-type breakdowns, duplicate/shared storage IDs, and cleanup candidates. First `geesome-node` aggregate helpers, AdminRead API routes, cached snapshot routes/table, queued async refresh path, file-catalog root/folder drilldown, group-post drilldown, and `geesome-ui` global screen exist; remaining work is generated-output DAG accounting, restored-data measurement, finer per-query progress, and deeper cleanup/detail views.
...
Status: in progress, with the first backend aggregate/API seams, cached snapshot routes/table, queued async refresh path, file-catalog root/folder drilldown endpoint, group-post drilldown endpoint, generated/static output source accounting, and `geesome-ui` global navigation/screen in place. The dedicated `storageSpace` module reports logical content bytes separately from deduplicated physical storage bytes for overview, type breakdown, top content, top file-catalog file/folder, top group, group-post, and DB-visible generated-output views; AdminRead routes expose live results under `admin/storage-space/*`; and a model-sync `StorageSpaceSnapshot` table can store timestamped analyzer snapshots for operator refreshes. The snapshot refresh now uses the shared async-operation queue runner under the `storage-space-snapshot` module for progress polling, while the database module keeps only the synced snapshot model and shared SQL connection. Remaining work is true IPFS DAG byte traversal for unknown generated refs, restored-data measurement, finer per-query progress, and deeper cleanup/detail views.
...
- Make long-running scans asynchronous for large nodes, with progress, timestamped cached results, and a refresh button. The first synchronous snapshot refresh route and queued async refresh path exist; remaining progress work is finer-grained stage/query reporting if restored data shows scans taking long enough to need it.

Likely backend contract:

- Add paged aggregate endpoints for file-catalog usage by root/folder, group usage by group/post attachments, top files by size, and type breakdowns by MIME/extension. The first database helper layer, AdminRead API exposure, cached latest/refresh snapshot routes, async refresh contract, file-catalog root/folder drilldown, group-post drilldown, and generated/static output source accounting exist; cleanup-specific drilldowns and true DAG byte traversal still need work.
