...
Status: in progress, with the first backend aggregate/API seams, cached snapshot routes/table, queued async refresh path, file-catalog root/folder drilldown endpoint, group-post drilldown endpoint, and `geesome-ui` global navigation/screen in place. `database.getStorageSpaceOverview`, type breakdown, top content, top file-catalog file/folder, top group, and group-post helpers now report logical content bytes separately from deduplicated physical storage bytes; AdminRead routes expose live results under `admin/storage-space/*`; and a model-sync `StorageSpaceSnapshot` table can store timestamped analyzer snapshots for operator refreshes. The snapshot refresh now uses the shared async-operation queue runner under the `storage-space-snapshot` module for progress polling while the database module owns only the storage-space job payload and snapshot calculation. Remaining work is generated-output DAG accounting, restored-data measurement, finer per-query progress, and deeper cleanup/detail views.
