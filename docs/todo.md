...
- ActivityPub-specific Sequelize models for actor, remote actor, follow, flag, object, and delivery records. Model-sync `ActivityPubActor` stores local group actor URLs and encrypted signing keys, `ActivityPubRemoteActor` stores fetched remote actor URL/key/inbox metadata, `ActivityPubFollow` stores inbound follow/cancel state, `ActivityPubFlag` stores pending moderation reports from signed `Flag` activities and backs a read-only admin list, `ActivityPubObject` stores local public post Note/Create IDs from outbox/post-object/publish-hook serialization plus signed remote `Create(Note)` replies and `Delete`/`Undo(Create)` tombstones for known local objects, and `ActivityPubDelivery` stores queued outbound follow `Accept` and post `Create(Note)` payloads plus retry/delivery/claim state; GeeSome post creation/deletion from remote object rows is still a future slice.
