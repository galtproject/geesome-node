...
- Native Bluesky/ATProto smoke and tests: status: public unauthenticated XRPC smoke for a configurable account exists as `npm run bluesky:atproto-smoke`, unit tests cover native ATProto feed URL/fetch injection plus `app.bsky.feed.post` facet/embed/reply projection into canonical rich text and source identity, module/API tests cover the read-only admin preview route, one-page async import route, read-only imported source-feed route, and bounded imported-post sync route, source-subscription tests cover model-backed subscribe/list/update/remove state, refresh tests cover direct import, queued processing, and due polling, and sync tests cover changed-CID re-import plus record-not-found soft deletion. Remaining: seeded `socNetAccount` harness for credentialed import/cross-post paths, review/import state, and idempotent GeeSome group/post storage.
