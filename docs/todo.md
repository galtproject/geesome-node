...
- `Content.isPinned` and `StorageObject.isPinned` still combine local pin intent with older remote-derived state. The new provider path no longer mutates them, but reconciliation must not clear legacy flags until local and remote provenance can be aggregated independently across every account sharing a `storageId`.
...
1. Define local-versus-remote pin provenance before deriving aggregate flags. Multiple accounts and users may claim the same `storageId`; confirmed remote claims may set a remote aggregate, but a missing claim must never clear another confirmed claim or an intentional local pin.
2. Add credential and pin-health operator flows after reconciliation is stable. Provide a secret-safe credential test, last successful check/error summaries, stale/failed counts, retry/reconcile actions, and bounded history. The UI should distinguish queued, provider-accepted, confirmed, stale, and failed states instead of presenting one optimistic pinned flag.
3. Add explicit status adapters before reconciling custom pin endpoints or additional providers; unsupported adapters must remain fail-closed and must not turn uncertainty into `missing`.
4. Prepare release-bound schema upgrades only when promoting `dev`: add migrations and migration-integrity checks for existing production tables, constraints, indexes, state backfills, and dedupe cleanup. Keep brand-new unreleased tables model-sync-only until that release boundary.
