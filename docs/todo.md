...
- Native Bluesky/ATProto smoke and tests: status: public unauthenticated XRPC smoke for a configurable account exists as `npm run bluesky:atproto-smoke`, and unit tests cover native ATProto feed URL/fetch injection plus `app.bsky.feed.post` facet/embed/reply projection into canonical rich text and source identity. Remaining: seeded `socNetAccount` harness for credentialed import/cross-post paths, delete/update mapping into stored GeeSome posts, review/import state, and idempotent GeeSome group/post storage.
