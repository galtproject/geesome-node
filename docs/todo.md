...
- Add a global UI menu item and screen for storage-space analysis: show which file catalogs and groups use how much space, then let operators drill into largest files, file types, duplicate/shared storage IDs, and cleanup candidates. Status: added as a dedicated planning slice; first `geesome-node` aggregate helpers, AdminRead API routes, cached snapshot routes/table, queued async refresh path, and the `geesome-ui` global screen exist. Remaining implementation should coordinate generated-output DAG accounting, restored-data measurement, finer per-query progress, and deeper folder/root drilldowns.
...
- Storage space analysis UI: add a global navigation entry and operator screen for file-catalog/group space usage, largest files, MIME/file-type breakdowns, duplicate/shared storage IDs, and cleanup candidates. First `geesome-node` aggregate helpers, AdminRead API routes, cached snapshot routes/table, queued async refresh path, and `geesome-ui` global screen exist; remaining work is generated-output DAG accounting, restored-data measurement, finer per-query progress, and deeper folder/root drilldowns.
...
Status: in progress, with the first backend aggregate/API seams, cached snapshot routes/table, queued async refresh path, and `geesome-ui` global navigation/screen in place. `database.getStorageSpaceOverview`, type breakdown, top content, top file-catalog file, and top group helpers now report logical content bytes separately from deduplicated physical storage bytes; AdminRead routes expose live results under `admin/storage-space/*`; and a model-sync `StorageSpaceSnapshot` table can store timestamped analyzer snapshots for operator refreshes. The snapshot refresh now uses the shared async-operation queue runner under the `storage-space-snapshot` module for progress polling while the database module owns only the storage-space job payload and snapshot calculation. Remaining work is generated-output DAG accounting, restored-data measurement, finer per-query progress, and deeper folder/root drilldowns.
