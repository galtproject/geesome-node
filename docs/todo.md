...
Status: post-MVP backlog in progress. Manual pinning, user automatic pinning, explicit group-post target policy, account UI, group settings UI, persistent pending-job deduplication, bounded cross-process policy-cache freshness, execution-time obsolete-job cancellation, bounded provider-call safety, and the shared dedupe lifecycle are shipped. The remaining items improve ownership clarity, remote drift handling, and larger account/post volume; they are not blockers for the completed single-node MVP.
...
1. Add the pin-account operator UI on the stable backend contract. Put credential testing, last successful check/error summaries, due/stale and failed counts, bounded history, and retry/reconcile actions in the account surfaces. Distinguish queued, provider-accepted, confirmed, stale, and failed states instead of presenting one optimistic pinned flag, and keep advanced diagnostics secondary to the primary account status and recovery action.
2. Add explicit status adapters before reconciling custom pin endpoints or additional providers; unsupported adapters must remain fail-closed and must not turn uncertainty into `missing`.
3. Prepare release-bound schema upgrades only when promoting `dev`: add migrations and migration-integrity checks for existing production tables, constraints, indexes, state backfills, and dedupe cleanup. Keep brand-new unreleased tables model-sync-only until that release boundary.
