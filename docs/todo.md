...
1. Add the opt-in periodic reconciliation scheduler and restart sweep around the proven queue methods. Keep conservative batch/rate limits, queue due rows before processing, expose env controls, and verify shutdown/restart recovery without duplicate provider calls.
2. Define local-versus-remote pin provenance before deriving aggregate flags. Multiple accounts and users may claim the same `storageId`; confirmed remote claims may set a remote aggregate, but a missing claim must never clear another confirmed claim or an intentional local pin.
3. Add credential and pin-health operator flows after reconciliation is stable. Provide a secret-safe credential test, last successful check/error summaries, stale/failed counts, retry/reconcile actions, and bounded history. The UI should distinguish queued, provider-accepted, confirmed, stale, and failed states instead of presenting one optimistic pinned flag.
4. Add explicit status adapters before reconciling custom pin endpoints or additional providers; unsupported adapters must remain fail-closed and must not turn uncertainty into `missing`.
5. Prepare release-bound schema upgrades only when promoting `dev`: add migrations and migration-integrity checks for existing production tables, constraints, indexes, state backfills, and dedupe cleanup. Keep brand-new unreleased tables model-sync-only until that release boundary.
