...
- `yarn audit --groups dependencies --level high` still reports high/critical transitive chains through older dependencies such as old IPFS packages, `sequelize-cli`, `geesome-libs`, and deprecated transitive `request` consumers. The Telegram bot `request` path is handled in [#819](https://github.com/galtproject/geesome-node/issues/819); the remaining `request`/`form-data`/`tough-cookie` path is from the `geesome-libs` wallet dependency chain.
