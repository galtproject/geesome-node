...
- Add user-facing source subscription/feed support. Status: geesome-ui now has the ActivityPub Sources reader, and backend source APIs resolve ActivityPub actors/handles, store subscription state independently from review/import decisions, manually refresh public `featured`/`outbox` source objects into the cache, and return sanitized feed items suitable for the reader. Remaining work is background-safe polling/follow behavior and cursor pagination if offset lists become too costly.
