...
- Native Bluesky/ATProto smoke and tests: status: public unauthenticated XRPC smoke for a configurable account exists as `npm run bluesky:atproto-smoke`, unit tests cover native ATProto feed URL/fetch injection plus `app.bsky.feed.post` facet/embed/reply projection into canonical rich text and source identity, module/API tests cover the read-only admin preview route and one-page async import route, source-subscription tests cover model-backed subscribe/list/update/remove state, and refresh tests cover direct import, queued processing, and due polling. Remaining: seeded `socNetAccount` harness for credentialed import/cross-post paths, delete/update mapping into stored GeeSome posts, review/import state, and idempotent GeeSome group/post storage.
