...
- `yarn audit --groups dependencies --level high` still reports high/critical transitive chains through older dependencies such as old IPFS packages, `sequelize-cli`, `geesome-libs`, and deprecated transitive `request` consumers. The Telegram bot `request` path is handled in [#819](https://github.com/galtproject/geesome-node/issues/819); the direct `geesome-libs` `node-forge` path is handled in [#821](https://github.com/galtproject/geesome-node/issues/821); the transitive `elliptic`, `secp256k1`, and `braces` advisories are handled in [#823](https://github.com/galtproject/geesome-node/issues/823), [#825](https://github.com/galtproject/geesome-node/issues/825), and [#826](https://github.com/galtproject/geesome-node/issues/826); consuming the merged `geesome-libs` crypto-resolution update is handled in [#827](https://github.com/galtproject/geesome-node/issues/827), consuming the wallet `axios`/`follow-redirects` update is handled in [#829](https://github.com/galtproject/geesome-node/issues/829), consuming the wallet `base-x`/`semver`/`ws` update is handled in [#831](https://github.com/galtproject/geesome-node/issues/831), the wallet `fetch-ponyfill`/`node-fetch` chain is handled in [#833](https://github.com/galtproject/geesome-node/issues/833) after the shared-library fix in `geesome-libs` [#138](https://github.com/galtproject/geesome-libs/issues/138), and the wallet `request`/`form-data` chain is handled in [#836](https://github.com/galtproject/geesome-node/issues/836) after the shared-library fix in `geesome-libs` [#140](https://github.com/galtproject/geesome-libs/issues/140); the remaining wallet `request` risk should be reduced by replacing the legacy `web3-provider-engine`/`request` stack rather than only patching leaf packages.
...
- Remaining high/critical alerts need larger migrations rather than direct lockfile bumps: the `telegram -> socks -> ip` chain and old `geesome-libs` wallet/libp2p crypto dependencies. After [#819](https://github.com/galtproject/geesome-node/issues/819), `request`, old `form-data`, and old `tough-cookie` alerts are no longer from `node-telegram-bot-api`; the visible remaining wallet path is the deprecated `geesome-libs -> geesome-wallet-client -> web3-provider-engine -> request` stack. After [#821](https://github.com/galtproject/geesome-node/issues/821), direct `geesome-libs` `node-forge` is patched, while the remaining `node-forge@0.10.0` path is the legacy `libp2p-crypto`/`peer-id` chain. After [#823](https://github.com/galtproject/geesome-node/issues/823) and [#825](https://github.com/galtproject/geesome-node/issues/825), the package-level `elliptic` and `secp256k1` critical advisories are removed; after [#831](https://github.com/galtproject/geesome-node/issues/831), [#833](https://github.com/galtproject/geesome-node/issues/833), and [#836](https://github.com/galtproject/geesome-node/issues/836), the wallet `ws`, `node-fetch`, and `form-data` leaf advisories are removed from the lockfile. The old `apidoc`/`apidoc-core` chain was removed separately in [#802](https://github.com/galtproject/geesome-node/issues/802).
