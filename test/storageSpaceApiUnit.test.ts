...
describe("storage space admin API", function () {
	it("requires admin read permission before reading storage-space aggregates", async () => {
		...
		const {call} = createCoreApiHarness({
			ms: {
				storageSpace: {
					...
					getStorageSpaceAvailabilityNetworkSamples: async () => {
						storageSpaceCalled = true;
						return {};
					},
					...
					queueStorageSpaceAvailabilityNetworkSampleRefresh: async () => {
						storageSpaceCalled = true;
						return {};
					},
					...
					getStorageSpaceSnapshotHistory: async () => {
						storageSpaceCalled = true;
						return {};
					},
		...
		const routes = [
			...
			["GET", "admin/storage-space/snapshots"],
	...
	it("exposes storage-space aggregate helpers through admin read routes", async () => {
		...
		const responses = {
			...
			snapshotHistory: [{id: 4, listLimit: 20}],
			...
			ms: {
				storageSpace: {
					...
					refreshStorageSpaceAvailabilityNetworkSamples: async (...args) => {
						...
						return responses.refreshedAvailabilitySamples;
					},
					queueStorageSpaceAvailabilityNetworkSampleRefresh: async (...args) => {
						storageSpaceCalls.push(["queueAvailabilityNetworkSamples", ...args]);
						return responses.queuedAvailabilitySamples;
					},
					...
					getStorageSpaceSnapshotHistory: async (listParams) => {
						storageSpaceCalls.push(["snapshotHistory", listParams]);
						return responses.snapshotHistory;
					},
		...
		assert.deepEqual((await call("GET", "admin/storage-space/snapshots", {user: {id: 7}, query})).body, responses.snapshotHistory);
